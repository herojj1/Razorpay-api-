...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8517366800:AAHwx_10zNvHYuKGdCAyiPFIf7BfFtArj24")
...
if __name__ == "__main__":
    asyncio.run(main())
  # =============================================================================
# PART 2 — Additional handlers (paste before main())
# =============================================================================

# ───────────── FAKE IDENTITY — EXTRA HANDLERS ─────────────
@client.on(events.CallbackQuery(pattern=rb"^fake_pick_(\w+)$"))
async def cb_fake_pick(event):
    """Pick a specific country from the inline list."""
    code = event.pattern_match.group(1).decode()
    ident = fake_data.get_fake(code)
    if not ident:
        return await event.answer("Unknown country", alert=True)
    await event.edit(pe(f"""{PE} <b>{bs('Full Name')}</b> ⌁ <code>{ident['name']}</code>
🏠 <b>{bs('Street')}</b> ⌁ <code>{ident['street']}</code>
🏙️ <b>{bs('City')}</b> ⌁ <code>{ident['city']}</code>
📍 <b>{bs('State')}</b> ⌁ <code>{ident['state']}</code>
...
💎 <i>{bs('Generated by NOXNI')}</i>"""),
    ...
    await event.answer(f"Generated {code}")


@client.on(events.CallbackQuery(pattern=rb"^fake_page_(\d+)$"))
async def cb_fake_page(event):
    """Paginated country list (16 per page)."""
    page = int(event.pattern_match.group(1).decode())
    codes = fake_data.all_country_codes()
    per_page = 16
    total_pages = math.ceil(len(codes) / per_page)
    ...
    for i in range(0, len(chunk), 4):
        row = [
            pbtn(c, data=f"fake_pick_{c}", style="primary")
            for c in chunk[i:i+4]
        ]
        kb.append(row)
    ...
    nav = []
    if page > 0:
        nav.append(pbtn("⬅️ Prev", data=f"fake_page_{page-1}", style="danger"))
    nav.append(pbtn(f"📄 {page+1}/{total_pages}", data="noop", style="primary"))
    if page < total_pages - 1:
        nav.append(pbtn("Next ➡️", data=f"fake_page_{page+1}", style="danger"))
    kb.append(nav)

    # Random + back
    kb.append([
        pbtn("🎲 Random Country", data="fake_rand", style="success"),
    ])
    kb.append([
        pbtn("🔙 Back", data="menu_fake", style="danger"),
    ])

    await event.edit(pe(f"""{PE} <b>{bs('Pick a Country')}</b>
{SEP}
🌍 {bs('Page')} <code>{page+1}</code>/<code>{total_pages}</code> · <code>{len(codes)}</code> {bs('countries')}
{SEP}
👇 {bs('Tap any country code')}"""),
        buttons=kb, parse_mode='html')
    await event.answer()
...
@client.on(events.CallbackQuery(data=b"fake_copy"))
async def cb_fake_copy(event):
    """Copy identity to clipboard (send as message)."""
    # Retrieve last generated from session cache
    last = SESSIONS.get(event.sender_id, {}).get("last_fake")
    if not last:
        return await event.answer("No identity to copy", alert=True)
    text = (f"Name: {last['name']}\n"
            f"Street: {last['street']}\n"
            f"City: {last['city']}\n"
            f"State: {last['state']}\n"
            f"Country: {last['country']}\n"
            f"Zip: {last['zip']}\n"
            f"Phone: {last['phone']}")
    await client_instance.send_message(event.sender_id, f"<code>{text}</code>",
                                        parse_mode='html')
    await event.answer("Sent to chat")


# ───────────── FAKE OVERRIDE (updated main /fake) ─────────────
# Replace the /fake handler from Part 1 with this enhanced version:

@client.on(events.NewMessage(pattern=r'^[/.]fake(?:\s+(.+))?$'))
async def cmd_fake_v2(event):
    arg = (event.pattern_match.group(1) or "").strip()

    # ── /fake or /fake list → paginated list ──
    if not arg or arg.lower() == "list":
        codes = fake_data.all_country_codes()
        per_page = 16
        total_pages = math.ceil(len(codes) / per_page)
        chunk = codes[:per_page]
        ...
        nav = [pbtn(f"📄 1/{total_pages}", data="noop", style="primary"),
               pbtn("Next ➡️", data=f"fake_page_1", style="danger")]
        kb.append(nav)
        kb.append([pbtn("🎲 Random", data="fake_rand", style="success")])
        kb.append([pbtn("🔙 Back", data="menu_fake", style="danger")])

        await styled_reply(event, pe(f"""{PE} <b>{bs('Fake Identity Generator')}</b>
{SEP}
✨ <b>{bs('Countries')}</b>: <code>{len(codes)}</code>
{SEP}
👇 {bs('Tap a country code below')}"""), buttons=kb)
        return
    ...
    # Cache last identity
    SESSIONS.setdefault(event.sender_id, {})["last_fake"] = ident
...
💎 <i>{bs('Generated by NOXNI')}</i>"""),
        buttons=[
            [pbtn("🎲 Another", data=f"fake_regen_{code}", style="success"),
             pbtn("🌍 Random", data="fake_rand", style="primary")],
            [pbtn("📋 Copy", data="fake_copy", style="primary"),
             pbtn("📖 All Countries", data="fake_page_0", style="primary")],
        ])


# Update regen to also cache
@client.on(events.CallbackQuery(pattern=rb"^fake_regen_(\w+)$"))
async def cb_fake_regen_v2(event):
    code = event.pattern_match.group(1).decode()
    ident = fake_data.get_fake(code)
    if not ident:
        return await event.answer("Error", alert=True)
...
@client.on(events.CallbackQuery(data=b"fake_rand"))
async def cb_fake_rand_v2(event):
    code = random.choice(fake_data.all_country_codes())
    ident = fake_data.get_fake(code)
    SESSIONS.setdefault(event.sender_id, {})["last_fake"] = ident
    await event.edit(pe(f"""{PE} <b>{bs('Full Name')}</b> ⌁ <code>{ident['name']}</code>
...
💎 <i>{bs('Generated by NOXNI')}</i>"""),
    ...
    await event.answer(f"Random: {code}")


@client.on(events.CallbackQuery(data=b"menu_fake_list"))
async def cb_fake_list_v2(event):
    """Redirect to paginated picker."""
    codes = fake_data.all_country_codes()
    per_page = 16
    total_pages = math.ceil(len(codes) / per_page)
    chunk = codes[:per_page]
    ...
    kb.append([pbtn(f"📄 1/{total_pages}", data="noop", style="primary"),
               pbtn("Next ➡️", data="fake_page_1", style="danger")])
    kb.append([pbtn("🎲 Random", data="fake_rand", style="success")])
    kb.append([pbtn("🔙 Back", data="menu_fake", style="danger")])

    await event.edit(pe(f"""{PE} <b>{bs('All Countries')}</b>
{SEP}
✨ <code>{len(codes)}</code> {bs('countries')}
{SEP}
👇 {bs('Tap a country code')}"""), buttons=kb, parse_mode='html')
    await event.answer()


# ───────────── ENHANCED FAKE MENU ─────────────
@client.on(events.CallbackQuery(data=b"menu_fake"))
async def cb_fake_menu_v2(event):
    await event.answer()
    codes = fake_data.all_country_codes()
    await event.edit(pe(f"""{PE} <b>{bs('Fake Identity')}</b>
{SEP}
👤 <b>{bs('Commands')}</b>
├─ <code>/fake US</code> — US identity
├─ <code>/fake random</code> — random country
└─ <code>/fake list</code> — browse all
{SEP}
🌍 <b>{bs('Total countries')}:</b> <code>{len(codes)}</code>
{SEP}
👇 {bs('Choose an action')}"""),
        buttons=[
            [pbtn("🎲 Random Identity", data="fake_rand", style="success")],
            [pbtn("📖 Browse All Countries", data="fake_page_0", style="primary")],
            [pbtn("🇺🇸 /fake US", data="fake_pick_US", style="primary"),
             pbtn("🇬🇧 /fake GB", data="fake_pick_GB", style="primary")],
            [pbtn("🇩🇪 /fake DE", data="fake_pick_DE", style="primary"),
             pbtn("🇮🇳 /fake IN", data="fake_pick_IN", style="primary")],
            [pbtn("🔙 Back", data="menu_home", style="danger")],
        ], parse_mode='html')


# ───────────── BULK FAKE (bonus) ─────────────
@client.on(events.NewMessage(pattern=r'^[/.]fakemass(?:\s+(\w+))?(?:\s+(\d+))?$'))
async def cmd_fakemass(event):
    code = (event.pattern_match.group(1) or "US").upper()
    n = max(1, min(500, int(event.pattern_match.group(2) or 10)))

    if code not in fake_data.FAKE_DATA:
        return await styled_reply(event, pe(
            f"❌ <b>{bs('Unknown country')}:</b> <code>{code}</code>"))

    lines = []
    for _ in range(n):
        i = fake_data.get_fake(code)
        lines.append(
            f"Name: {i['name']}\n"
            f"Street: {i['street']}\n"
            f"City: {i['city']}\n"
    ...
    fn = f"fake_{code}_{int(time.time())}.txt"
    async with aiofiles.open(fn, "w", encoding="utf-8") as f:
        await f.write("\n".join(lines))
    try:
        await client_instance.send_file(
            event.sender_id, fn,
            caption=pe(f"{PE} <b>{bs('Fake Identities')}</b> · {code} · {n}"))
    finally:
        try: os.remove(fn)
        except Exception: pass


@client.on(events.NewMessage(pattern=r'^[/.]fakelist$'))
async def cmd_fakelist(event):
    codes = fake_data.all_country_codes()
    half = (len(codes) + 1) // 2
    left = "  ".join(f"<code>{c}</code>" for c in codes[:half])
    right = "  ".join(f"<code>{c}</code>" for c in codes[half:])
    await styled_reply(event, pe(f"""{PE} <b>{bs('All Countries')}</b>
{SEP}
<code>{len(codes)}</code> {bs('total')}
{SEP}
{left}
...
💡 <code>/fake US</code>"""),
        buttons=[
            [pbtn("🎲 Random", data="fake_rand", style="success"),
             pbtn("📖 Browse", data="fake_page_0", style="primary")],
            [pbtn("🔙 Menu", data="menu_home", style="danger")],
        ])


# ───────────── ADMIN — FAKE STATS ─────────────
@client.on(events.NewMessage(pattern=r'^[/.]fakestats$'))
async def cmd_fakestats(event):
    if event.sender_id not in ADMIN_ID:
        return
    codes = fake_data.all_country_codes()
    # Count total cities/streets/names across all countries
    total_cities = sum(len(v["cities"]) for v in fake_data.FAKE_DATA.values())
    total_streets = sum(len(v["streets"]) for v in fake_data.FAKE_DATA.values())
    total_first = sum(len(v["first"]) for v in fake_data.FAKE_DATA.values())
    total_last = sum(len(v["last"]) for v in fake_data.FAKE_DATA.values())
    await styled_reply(event, pe(f"""{PE} <b>{bs('Fake Data Stats')}</b>
{SEP}
🌍 <b>{bs('Countries')}:</b> <code>{len(codes)}</code>
🏙️ <b>{bs('Cities')}:</b> <code>{total_cities}</code>
🛣️ <b>{bs('Streets')}:</b> <code>{total_streets}</code>
👤 <b>{bs('First names')}:</b> <code>{total_first}</code>
👥 <b>{bs('Last names')}:</b> <code>{total_last}</code>
{SEP}
💎 <b>{bs('Total combinations')}:</b> <code>{total_first * total_last * total_cities * total_streets:,}</code>
{SEP}
📋 {bs('Sample countries')}:
<code>{', '.join(codes[:20])}</code>"""))
...
async def cleanup_sessions_loop():
    ...
    while True:
        ...
        try:
            now = time.time()
            expired = [uid for uid, s in SESSIONS.items()
                       if now - s.get("ts", 0) > 3600]
            for uid in expired:
                SESSIONS.pop(uid, None)
            if expired:
                log_system("CLEANUP", f"removed {len(expired)} sessions")
        except Exception as e:
            log_system("CLEANUP", f"{e!r}", "error")


# ───────────── UPDATED MAIN (starts cleanup loop) ─────────────
async def main():
    global client_instance
    client_instance = client
    log_system("BOOT", "Starting NOXNI v2.2")
