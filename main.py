"""
Shopify Checker API — All-in-One (v3.2.0)
==========================================
...
v3.2.0:
  ...
  - Strict 5-point charge verification (no fake CHARGED)
...
def _proposal_body(ses: str, stable: str, variant: str, queue: Optional[str],
    ...
    payload = {
        "variables": {
            ...
            "queueToken": queue,  # None → JSON null (correct)
...
def submit_payment(client: TLS, shop: str, ref: str, ck_tok: str, ses_tok: str,
    ...
    # Send real signed handles — required by Shopify to bypass delivery-change gate
            ...
            try:
                ...
                if rtype == "FailedReceipt":
                    ...
                    if ("SOME_DELIVERY_DETAILS_MAY_HAVE_CHANGED" in code
                        ...
                        try:
                            # Re-propose to get fresh handles
...
app = FastAPI(title="Shopify Checker API", version="3.2.0", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.2.0</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.2.0"}
