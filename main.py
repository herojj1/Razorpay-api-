"""
Shopify Checker API — All-in-One (v3.0.1)
==========================================
v3.0.1:
  - FIXED: actions regex `(?:...)` → `(...)` — capturing group for _g()
  - DEFENSIVE: _g() now handles patterns with no capture group
  - All v3.0.0 features preserved (strict charge verification, 200 threads)

CHARGED only when Shopify confirms a real order (with /orders/ URL or Order GID).
"""
...
_R = {
    ...
    # ─── FIXED: capturing group ( ) instead of non-capturing (?: ) ───
    "actions":      re.compile(
        r'('
...
def _g(rx: str, s: str, default: str = "") -> str:
    ...
    v3.0.1: handles patterns with no capture group (returns group(0)).
    """
    try:
        pat = _R[rx]
        m = pat.search(s)
...
def is_real_charge(poll_body: str) -> Tuple[bool, str]:
    ...
    STRICT verification — CHARGED only when ALL of these hold:
      ...
      3. A real /orders/ URL exists OR an Order GID is present
    """
    try:
...
app = FastAPI(title="Shopify Checker API", version="3.0.1", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.0.1</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.0.1"}
