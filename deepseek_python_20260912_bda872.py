...
v2.2.0:
  - Fixed Step 3 actions JS regex for new Shopify CDN patterns
  - Added fallback URL detection
  - Handles absolute CDN URLs
...
# ─────────────────────────── Step 3: actions JS (FIXED) ─────────────────
def fetch_ids(client: TLS, shop: str, checkout_html: str) -> Tuple[str, str, str]:
    """
    Fetch the actions JS and extract proposal/submit/poll IDs.
    v2.2.0: Now handles new Shopify CDN URL patterns.
    """
    url = _g("actions", checkout_html)

    # ─── Fallback: try broader patterns if primary regex missed ───
    if not url:
        fallbacks = [
            r'(https://cdn\.shopify\.com/shopifycloud/checkout-web/assets/[^"\']+\.js)',
            r'(/cdn/shopifycloud/checkout-web/assets/[^"\']+\.js)',
            r'(/assets/[^"\']*actions[^"\']*\.js)',
            r'(https://[^"\']*checkout-web[^"\']*\.js)',
        ]
        for pattern in fallbacks:
            m = re.search(pattern, checkout_html)
            if m:
                url = m.group(1)
                log.warning("Step3 fallback matched: %s", url[:120])
                break

    if not url:
        # Last resort: dump a snippet of the HTML for debugging
        snippet = checkout_html[:500].replace("\n", " ")
        log.error("Step3 failed — checkout HTML snippet: %s", snippet)
    ...
    if r.status_code != 200:
        raise Exception(f"actions js HTTP {r.status_code} from {full_url[:80]}")
    ...
    if not prop or not subm:
        # Broader fallback: search for any 64-char hex near keywords
        if not prop:
            m = re.search(r'name:"Proposal"[^}]{0,200}?id:"([a-f0-9]{64})"', js) or \
                re.search(r'id:"([a-f0-9]{64})"[^}]{0,200}?name:"Proposal"', js)
            if m:
                prop = m.group(1)
        if not subm:
            m = re.search(r'name:"SubmitForCompletion"[^}]{0,200}?id:"([a-f0-9]{64})"', js) or \
                re.search(r'id:"([a-f0-9]{64})"[^}]{0,200}?name:"SubmitForCompletion"', js)
            if m:
                subm = m.group(1)
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        ...
        try:
            ...
            cur = _g("cur", resp) or cur
            ctry = _g("ctry", resp) or ctry
            result.currency = cur
            qt = _g("queue", resp)
            if not qt:
...
app = FastAPI(title="Shopify Checkout API", version="2.2.0", docs_url=None, redoc_url=None)
...
<h1>Shopify Real-Charge API v2.2.0</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "2.2.0"}
