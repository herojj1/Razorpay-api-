import json
"""
VXO Checker — Shopify Checkout Engine (v2.2.1)
=============================================
Merged + hardened version.

Change log v2.2.1 (from v2.2.0):
    - FIX: removed invalid "terms" field from NegotiationInput (was causing
           INVALID_VARIABLE on every SubmitForCompletion call)
    - FIX: Step 6 country sync + final_addr tracking (shipping fallback)
    - FIX: HardDecline class for non-retryable declines (CARD_DECLINED etc.)
    - FIX: removed hardcoded poll_for_receipt_id fallback
    - FIX: is_real_charge requires non-zero total
    - FIX: real _redact() implementation (masks PANs, tokens)

Change log v2.2.0 (from v2.1.0):
    - normalize_proxy(): handles /addproxy command form; fails cleanly
    - COUNTRY_ADDRESSES: corrected zone codes per country
    - extract_checkout_total(): reads from sellerProposal only
    - extract_shipping_amount(): reads from sellerProposal explicitly
    - send_proposal / send_proposal2: real billingAddress populated
    - CAPTCHA_REQUIRED now retryable with proxy rotation
    - find_cheapest_product: respects Retry-After on 429
    - is_proxy_alive(): pre-flight proxy health check
    - Retry loop also catches PAYMENTS_UNACCEPTABLE_PAYMENT_AMOUNT
"""
...
class HardDecline(Exception):
    """Non-retryable decline raised by proposal/submit checks."""
    pass
...
# ──────────────────────── Address database (corrected zones) ─────────
...
def is_real_charge(poll_body: str) -> Tuple[bool, str]:
    ...
    # Only accept order identifiers if a non-zero total was already verified above.
...
def _rotate_client(current_proxy: str, attempt: int) -> Tuple[str, str, str]:
    new_proxy = current_proxy
    if PROXY_POOL and attempt > 1:
        candidates = [p for p in PROXY_POOL if p != current_proxy] or PROXY_POOL
        new_proxy = random.choice(candidates)
    return new_proxy, random.choice(BROWSER_PROFILES), random.choice(USER_AGENTS)
...
def extract_shipping_amount(proposal_body: str) -> str:
    """Read shipping from the seller's selected strategy in sellerProposal."""
    try:
        data = json.loads(proposal_body)
        seller = (data.get("data", {}).get("session", {}).get("negotiate", {})
                      .get("result", {}).get("sellerProposal", {}))
...
def extract_checkout_total(proposal_body: str) -> str:
    """Read total from sellerProposal only."""
    try:
        data = json.loads(proposal_body)
        seller = (data.get("data", {}).get("session", {}).get("negotiate", {})
                      .get("result", {}).get("sellerProposal", {}))
...
# v2.2.1 FIX: the "terms" field is NOT defined on NegotiationInput.
# It was removed entirely from the payload below. The signedHandles in
# deliveryExpectations already satisfy Shopify's terms negotiation.
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        ...
        # Step 6 — with country sync + final_addr tracking
        try:
            addr = pick_address_for_store(shop_url, currency, country)
        ...
        # Step 11 — Poll for receipt (30 attempts) + real-charge check
