import json
"""
VXO Checker — Shopify Checkout Engine (v2.1.0)
=============================================
Pure checkout automation engine for the VXO Checker API.

Change log v2.1.0:
    - Added is_real_charge() — CHARGED only fires when a real order
      exists in the receipt. Prevents fake CHARGED on auth-only receipts.
    - Poll cap raised from 20 to 30.
    - Downgrade logging when a receipt can't be verified.
...
Flow:
    ...
    Step 11 — Poll for receipt (CHARGED / APPROVED / DECLINED) + real-charge check
"""
...
def is_real_charge(poll_body: str) -> Tuple[bool, str]:
    """
    Return (is_real, receipt_url).

    A "real" charge requires ALL of:
      1. A receipt object with a non-empty purchaseOrder
      2. A paid amount > $0.00
      3. At least one strong signal of an actual order:
           - confirmationPage.url contains "/orders/"
           - orderStatusPageUrl contains "/orders/"
           - an order id in gid://shopify/Order/<n> form
           - purchaseOrder.orderNumber / name / id is present

    If any check fails, this is NOT a real charge.
    """
    try:
    ...
    # 1. purchaseOrder must exist
    po = receipt.get("purchaseOrder")
    if not po:
        return False, ""

    # 2. amount paid must be > 0
    total_paid = (
    ...
    # 3a. confirmationPage.url with /orders/
    ...
    # 3b. orderStatusPageUrl with /orders/
    osp = receipt.get("orderStatusPageUrl") or ""
    if osp and "/orders/" in osp:
        return True, osp

    # 3c. order id in gid://shopify/Order/<n>
    ...
    # 3d. purchaseOrder has any identity field
