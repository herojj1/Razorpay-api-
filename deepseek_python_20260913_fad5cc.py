"""
Shopify Checker API — All-in-One (v3.2.1)
==========================================
...
class TLS:
    """
    v3.2.1 — TLS client with automatic non-impersonated fallback on proxy 407.
    ...
    """
    def __init__(self, proxy: str = "", timeout: int = 20, impersonate: bool = True):
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        # Step 0 — v3.2.1: tag proxy errors distinctly
...
app = FastAPI(title="Shopify Checker API", version="3.2.1", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.2.1</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.2.1"}
