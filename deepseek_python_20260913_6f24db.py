"""
Shopify Checker API — All-in-One (v3.0.2)
==========================================
v3.0.2:
  - DEBUG: Step 4 now includes the raw Shopify response in the error message
  - This tells us WHY the proposal was rejected (captcha, checkpoint, etc.)

v3.0.1:
  - FIXED: actions regex `(?:...)` → `(...)` — capturing group for _g()
  - DEFENSIVE: _g() now handles patterns with no capture group

CHARGED only when Shopify confirms a real order.
"""
...
def run_checkout(shop: str, card_entry: str, proxy: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        ...
        try:
            ...
            qt = _g("queue", resp)
            if not qt:
                # Extract the most useful error signal from the response
...
app = FastAPI(title="Shopify Checker API", version="3.0.2", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.0.2</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.0.2"}
