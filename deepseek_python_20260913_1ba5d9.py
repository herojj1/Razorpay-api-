"""
Shopify Checker API — All-in-One (v3.1.0)
==========================================
FULLY WORKING VERSION — all bugs fixed.

v3.1.0:
  - CRITICAL FIX: Added missing "id" field to Proposal payloads
  - CRITICAL FIX: Added missing "id" field to SubmitForCompletion payload
  - FIXED: queueToken null vs empty string
  - FIXED: Removed cartMetafields (rejected by older Shopify stores)
  - FIXED: actions regex uses capturing group, not non-capturing
  - Defensive _g() handles 0-group patterns
...
def _proposal_body(ses: str, stable: str, variant: str, queue: Optional[str],
    ...
    """v3.1.0: prop_id added — required for Shopify persisted queries."""
    if addr:
        partial = {
    ...
    payload = {
        ...
        "id": prop_id,  # ← CRITICAL FIX — was missing
    }
...
def submit_payment(client: TLS, shop: str, ref: str, ck_tok: str, ses_tok: str,
    ...
    payload = {
        ...
        "id": submit_id,  # ← CRITICAL FIX — was missing
    }
    ...
    try:
        ...
        try:
            ...
            if not qt:
                ...
                err_sig = ""
                for pat_rx in (r'"code"\s*:\s*"([^"]+)"',
                               r'"nonLocalizedMessage"\s*:\s*"([^"]+)"',
                               r'"message"\s*:\s*"([^"]+)"'):
                    mm = re.search(pat_rx, resp)
                    if mm:
                        err_sig = mm.group(1)
                        break
                snippet = resp[:200].replace("\n", " ").replace('"', "'")
                raise Exception(f"no queue | typename={typename} | sig={err_sig} | resp={snippet}")
        except Exception as e:
            result.retryable = True
...
app = FastAPI(title="Shopify Checker API", version="3.1.0", docs_url=None, redoc_url=None)
...
<h1>Shopify Checker API v3.1.0</h1>
...
async def health():
    with _lock:
        a = _active
    return {"ok": True, "threads": THREADS, "retries": RETRIES, "active": a, "version": "3.1.0"}
